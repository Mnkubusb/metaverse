...
Secrets are never committed or baked into Docker images. On the server, create `~/metaverse.env`
(readable only by the deploy user, `chmod 600`) with:

```bash
DATABASE_URL=postgresql://...
JWT_SECRET=<at least 32 random characters, e.g. from `openssl rand -base64 48`>
ALLOWED_ORIGIN=https://your-frontend.example
```

The deploy workflows start the containers with `--env-file ~/metaverse.env`. With `NODE_ENV=production`
(set in the images), the servers refuse to start with a `JWT_SECRET` shorter than 32 characters.
Run `pnpm db:migrate` and `pnpm db:seed` against the production database after schema changes.
